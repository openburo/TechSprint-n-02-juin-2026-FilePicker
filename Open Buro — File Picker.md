...
The **File Picker** is the first Open Buro standard. It defines a protocol that lets a *consumer* application (mail, docs, chat, calendar…) cast an *intent* asking a *provider* application (a drive such as Twake Drive, Fichiers/DINUM, Nextcloud…) to present a file-selection surface and return the result in a standard shape. The provider supplies its own picker frontend; the consumer rebuilds nothing and inherits the provider's full navigation (search, favourites, recents, tags…).

This document is the **explanatory specification** of the File Picker: its purpose, terminology, architecture, and the intent lifecycle. The deep normative artefacts (the formal manifest JSON Schema, the exhaustive `postMessage` message catalogue, the error registry, and the security model) are presented here at the architectural level and **reserved** for iterative refinement; the sections that hold them are marked `[RESERVED]`.

---

## Status of This Document

This is an **Editor's Draft**. Open Buro and its File Picker standard are in **beta**. The protocol described here was validated in practice during the April 2026 TechSprint, where four independent providers (Jalios, a WebDAV picker, Fichiers/DINUM, and Twake Drive) interoperated against a shared capabilities registry.
...
The need for a service-orchestration standard emerged from concrete collaboration between **La Suite numérique (DINUM)** and **Twake**, who worked on advanced cross-service integration. The File Picker was the first use case taken to a TechSprint (April 2026), producing the reference material this document formalises.
...
| Term | Definition |
|---|---|
| **Consumer** | The application that initiates a file-picking interaction by casting an intent (e.g. a mail or document editor). It rebuilds no picker UI. |
| **Provider** | The application that exposes a file-picking surface and resolves the intent (e.g. a drive). It supplies its own frontend. |
| **Capability** | A declared ability of a provider to handle a class of intent — for the File Picker: `PICK`, `SAVE`, or `SHARE`. |
...
| **Capabilities file** | The standard-format document in which an application declares its capabilities (see §5.2). |
...
| **Open Buro Bridge** | The reference client library that handles the consumer side of the intent lifecycle (discovery, resolution, iframe and `postMessage` orchestration, answer management). |
...
The Open Buro File Picker is a protocol that lets a **consumer** cast an **intent** asking to open a file picker served by a **provider**. From the end-user's standpoint: while attaching a file in a mail client (consumer), the user is presented with their drive's (provider's) own picker, browses it natively, selects, and the result flows back into the mail client.

Defining properties:

- The **picker frontend is supplied by the provider**. The consumer rebuilds nothing.
- All of the provider's navigation features (search, favourites, recents, tags…) are **automatically available**.
- The picker interface is **homogeneous** from one consumer to the next — it is the provider's picker, wherever it is invoked.
...
- A consumer can **list the providers** that expose a file-picker capability — `PICK`, `SAVE`, or `SHARE`.

### 3.2 Roles at a glance

| Role | Responsibility |
|---|---|
| **Consumer** | Casts the intent; drives the lifecycle (with the Open Buro Bridge); consumes the answer. |
...
**This document addresses workplace administrators and application developers.** The product ambition remains centred on the end-user: an integrated, fluid, productive journey.
...
### 5.2 The capabilities file (manifest)

Every application in the ecosystem — consumer or provider — **MUST** declare its capabilities in a **capabilities file** whose format is part of the standard. Providers declare the file-picker capabilities they serve. The shape validated at the TechSprint, adopted here as canonical, is:
...
```json
{
  ...
  "capabilities": [
    ...
    },
    {
      "action": "SHARE",
      "properties": { "mimeTypes": ["*/*"] },
      "path": "https://drive.example.eu/capabilities/SHARE"
    }
  ]
}
```

Field intent: `id`/`name`/`version` identify the provider; `url` locates the capabilities file itself; each entry in `capabilities[]` binds an `action` (`PICK` | `SAVE` | `SHARE`) to a `path` (the frontend surface to embed) and a set of `properties` (e.g. accepted `mimeTypes`, `multiple`); optional `display` hints at presentation (e.g. `modal`).
...
The list of available capabilities is served by a minimal **platform registry** (the "Open Buro server") that exposes the capabilities of the workplace's applications. A consumer queries it to discover which providers expose `PICK`, `SAVE`, or `SHARE`.

**Out of scope (see §8.1):** *how* applications come to know the link to each other's capabilities file. A static configuration and a dynamic registry are both compatible; the discovery transport is not constrained here.
...
- returns the answer to the consumer through a callback.
...
The consumer owns the lifecycle: **intent cast → retrieve capabilities → capability chooser → iframe + `postMessage` setup → iframe layout → answer management**. The Open Buro Bridge implements these steps.
...
|---|---|---|---|
| `action` | string | all | `PICK`, `SAVE`, or `SHARE`. |
...
| `data` | object | SAVE, SHARE | Action input, e.g. for SAVE `{ content, filename }`. |
...
Deposit content supplied by the consumer into a location the user chooses in the provider (the provider serves a destination-selection surface). The consumer supplies `data: { content, filename }`; the response confirms the stored item.

### 7.3 SHARE

Hand an item to the provider so the provider can produce a share artefact (e.g. a nominative access link) according to its own sharing policy, and return that artefact to the consumer.

> `[RESERVED]` **SHARE — parameter and response schema.** SHARE is normative in V1, but its `data` inputs (what the consumer hands over) and its response shape (the share artefact, its scope, lifetime, and revocation semantics) are richer than PICK/SAVE and were not exercised at the April TechSprint. They will be specified before V1 is declared stable; until then, treat SHARE's payloads as provisional. *(Per the postpone principle: SHARE's role is fixed now; its detailed payloads are postponed with this explicit flag.)*
...
**In scope:** the protocol by which a consumer casts an intent, discovers and chooses a provider, embeds the provider's picker, and receives a standard answer.
...
**Why this shape.** The TechSprint confirmed that a workable open file picker needs: a normalised-but-rich response, MIME filtering, multi-select, a clear PICK/SAVE (and now SHARE) distinction, and an isolation model where the provider serves its own UI and the consumer never gains ambient access to provider data. The full comparative tables live in the April working note for readers who want the detail.
